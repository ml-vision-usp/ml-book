...
O algorítmo backpropagation é uma forma eficiente de se computar o gradiente ![](./imagens/nabla_L.png) de uma função de perda que uma rede neural possui. Com frequência utilizamos o erro quadrático médio:
...
$$
    \frac{\partial e}{\partial w_{ij}^{(l)}} = \bold{x}_{i}^{(l-1)} \cdot \bold{\delta}_{j}^{(l)}
$$
...
$$
    \frac{\partial e}{\partial \bold{s}^{(l)}} = \frac{\partial e}{\partial \bold{x}^{(l)}} \cdot \frac{\partial \bold{x}^{(l)}}{\partial \bold{s}^{(l)}}
$$

$$
    \bold{\delta}_{j}^{(l)} = \frac{\partial e}{\partial\bold{s}_{j}^{(l)}} = \frac{\partial e}{\partial \bold{x}_{j}^{(l)}} \cdot \frac{\partial \bold{x}_{j}^{(l)}}{\partial\bold{s}_{j}^{(l)}}
$$
